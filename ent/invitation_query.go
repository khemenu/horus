...
func (iq *InvitationQuery) WithInviter(opts ...func(*AccountQuery)) *InvitationQuery {
	query := (&AccountClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(invitation.FieldDateCreated).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Invitation.Query().
//		Select(invitation.FieldDateCreated).
//		Scan(ctx, &v)
...
func (iq *InvitationQuery) loadInviter(ctx context.Context, query *AccountQuery, nodes []*Invitation, init func(*Invitation), assign func(*Invitation, *Account)) error {
	...
	nodeids := make(map[uuid.UUID][]*Invitation)
	for i := range nodes {
		if nodes[i].account_invitations == nil {
			continue
		}
		fk := *nodes[i].account_invitations
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
