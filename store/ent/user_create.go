...
func (uc *UserCreate) SetAuthorizerID(id int) *UserCreate {
	uc.mutation.SetAuthorizerID(id)
	return uc
}

// SetNillableAuthorizerID sets the "authorizer" edge to the Authorizer entity by ID if the given value is not nil.
func (uc *UserCreate) SetNillableAuthorizerID(id *int) *UserCreate {
	if id != nil {
		uc = uc.SetAuthorizerID(*id)
	}
	return uc
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.AuthorizerIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2O,
			Inverse: false,
