...
// QueryAuthorizer chains the current query on the "authorizer" edge.
func (uq *UserQuery) QueryAuthorizer() *AuthorizerQuery {
	query := (&AuthorizerClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithAuthorizer tells the query-builder to eager-load the nodes that are connected to
// the "authorizer" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithAuthorizer(opts ...func(*AuthorizerQuery)) *UserQuery {
	query := (&AuthorizerClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) loadTokens(ctx context.Context, query *TokenQuery, nodes []*User, init func(*User), assign func(*User, *Token)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(token.FieldOwnerID)
	}
...
func (uq *UserQuery) loadAuthorizer(ctx context.Context, query *AuthorizerQuery, nodes []*User, init func(*User), assign func(*User, *Authorizer)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(authorizer.FieldOwnerID)
	}
	query.Where(predicate.Authorizer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.AuthorizerColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
