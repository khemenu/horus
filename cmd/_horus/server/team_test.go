...
func TestNewTeam(t *testing.T) {
	t.Run("as an org owner", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		rst, err := h.Orgs().New(ctx, horus.SiloInit{
			OwnerId: h.user.Id,
			Alias:   "Khepri",
		})
		require.NoError(err)
	...
	t.Run("as an org member", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		...
		org, err := h.Orgs().New(ctx, horus.SiloInit{
			OwnerId: other.Id,
			Alias:   "Khepri",
		})
		require.NoError(err)
...
func TestInviteMember(t *testing.T) {
	...
	t.Run("as a team owner, invite a member in another org", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		...
		rst2, err := h.Orgs().New(ctx, horus.SiloInit{
			OwnerId: h.user.Id,
			Alias:   "Khepri",
		})
		require.NoError(err)
...
func TestDeleteTeamMember(t *testing.T) {
	t.Run("team does not exist", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		...
		team_id := uuid.New()
		_, err = h.client.DeleteTeamMember(ctx, &pb.DeleteTeamMemberReq{
			TeamId:   team_id[:],
			MemberId: rst.Owner.Id[:],
		})
		s, ok := status.FromError(err)
	...
	t.Run("delete team member as a team owner", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		other, err := h.Users().New(ctx)
		require.NoError(err)
	...
	t.Run("delete team owner as a team owner", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		other, err := h.Users().New(ctx)
		require.NoError(err)
...
func TestDeleteTeam(t *testing.T) {
	t.Run("team does not exist", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		team_id := uuid.New()
		_, err := h.client.DeleteTeam(ctx, &pb.DeleteTeamReq{
			TeamId: team_id[:],
		})
		require.NoError(err)
	}))
	...
	t.Run("as an org owner", WithHorusGrpc(func(require *require.Assertions, ctx context.Context, h *horusGrpc) {
		...
		_, err = h.client.DeleteTeam(ctx, &pb.DeleteTeamReq{TeamId: team.Id[:]})
		require.NoError(err)
